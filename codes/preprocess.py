...
def feature_engineer_pl(x, grp, 
    ...
    if level_diff:
        ...
        df_level_diff_summary = df_level_diff.groupby('session_id').agg([
            ...
            *[pl.col(f'elapsed_time_level_diff_{n}').apply(lambda s: s if s < 0 else 0, return_dtype='Int64').alias(f'elapsed_fime_level_diff_fixed_{n}') for n in [i for i in LEVELS if i not in [4,13,22]]],
            pl.col('elapsed_time_diff_max0-3_min4').apply(lambda s: s if s > 0 else 0, return_dtype='Int64').alias('elapsed_time_diff_fixed_max0_3_min4'),
            pl.col('elapsed_time_diff_max5-11_min12').apply(lambda s: s if s > 0 else 0, return_dtype='Int64').alias('elapsed_time_diff_fixed_max5-11_min12'),
            pl.col('elapsed_time_diff_max13-21_min22').apply(lambda s: s if s > 0 else 0, return_dtype='Int64').alias('elapsed_time_diff_fixed_max13-21_min22'),
        ).drop(
